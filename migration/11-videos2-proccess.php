...
function create_partial_table($truncate = FALSE) {
    ...
    $sql = "CREATE TABLE IF NOT EXISTS `TVideoTipo11` (
    ...
    $sql = "CREATE TABLE IF NOT EXISTS `TVideoCompanies11` (
    ...
    $sql = "CREATE TABLE IF NOT EXISTS `TVideoCountries11` (
    ...
    $sql = "CREATE TABLE IF NOT EXISTS `TVideoFile11` (
    ...
    $sql = "CREATE TABLE IF NOT EXISTS `TVideoSecciones11` (
    ...
    #Tabla TVideo
    $sql = "CREATE TABLE IF NOT EXISTS `TVideo11` (
    ...
    if ($q6 === TRUE) {
        echo "\033[1;32m"; echo "✔ Tablas 11 creadas.\n"; echo "\033[0m";
    } else {
    ...
    if ($truncate) {
        $q5 = $conn->query("TRUNCATE TABLE TVideoTipo11; TRUNCATE TABLE TVideoCompanies11; TRUNCATE TABLE TVideoCountries11; TRUNCATE TABLE TVideoFile11; TRUNCATE TABLE TVideoSecciones11; TRUNCATE TABLE TVideo11;");
        echo "\033[1;32m"; echo "✔ Tablas 11 limpias.\n"; echo "\033[0m";
    }
}
...
function create_itermediate_table($truncate = FALSE) {
    ...
    $table_video = VIDEO2_INTERMEDIATE_TABLE;
...
function create_type_videos() {
    ...
    $sql = "SELECT * FROM `TVideoTipo11` WHERE Estado = '1' ORDER BY IdTipo ASC;";
    $result = $conn->query($sql);
...
function get_videos_from_partial($from_id = 1) {
    ...
    $sql = "SELECT TVideo11.*, TVideoFile11.VimeoST, TVideoFile11.VimeoHD, TVideoFile11.VimeoMovil, TVideoFile11.VimeoEmbed, TVideoFile11.Link
            FROM TVideo11
            INNER JOIN TVideoFile11 ON TVideoFile11.VideoID = TVideo11.IdVideo
            WHERE Activo = 1 AND TVideoFile11.Link != '' AND TVideoFile11.VimeoST IS NULL AND TVideoFile11.VimeoHD IS NULL
            AND TVideo11.IdVideo >= '$from_id'
            GROUP BY TVideo11.IdVideo
            ORDER BY TVideo11.IdVideo ASC, TVideoFile11.IdVideoFile ASC;";
    $result = $conn->query($sql);
...
function create_videos_on_WP($limit = FALSE, $inactive = FALSE, $just_id = FALSE) {
    ...
    $conn = connect_to_partial();
    $conn->set_charset("utf8");
    $base_url = "https://quick.produ.com";

    if ($data) {
        foreach ($data as $key => $item) {
            $url = $item->Link;
            if ($item->Titulo && $url !== NULL && $url !== '') {
                ...
                if (strpos(strtolower($url), 'av/') !== FALSE) {
                    $stripped_url = substr($url, strpos($url, 'av/'));
                    $new_url = "https://quick.produ.com/" . $stripped_url;
                    ...
                    if ($post_id) {
                        ...
                        #Empresas
                        $companies = [];
                        $sql = "SELECT * FROM TVideoCompanies11 WHERE VideoID = '$item->IdVideo' ORDER BY IdVideoCompany ASC;";
                        ...
                        $countries = [];
                        if ($dictionary) {
                            $sql = "SELECT * FROM TVideoCountries11 WHERE VideoID = '$item->IdVideo' ORDER BY IdVideoCountry ASC;";
                        ...
                        if ($category_raw) {
                            ...
                            $sql = "SELECT * FROM TVideoSecciones11 WHERE VideoID = '$item->IdVideo' ORDER BY IdVideoSeccion ASC LIMIT 1;";
                        ...
                        update_field('embed', FALSE, $post_id);
                        update_field('url', $new_url, $post_id);
...
function init() {
    ...
    // get_file('TVideo', 'TVideo11', FALSE, FALSE);
    // get_file('TVideoFile', 'TVideoFile11', FALSE, FALSE);
    // get_file('TVideoCountries', 'TVideoCountries11', FALSE, FALSE);
    // get_file('TVideoCompanies', 'TVideoCompanies11', FALSE, FALSE);
    // get_file('TVideoTipo', 'TVideoTipo11', FALSE, FALSE);
    // get_file('TVideoSecciones', 'TVideoSecciones11', FALSE, FALSE);

    #Cargar data a partial desde archivos
    // for ($i = 1; $i <= FILE_PARTS; $i++) {
    //     load_data('TVideo11', $i);
    //     sleep(15);
    // }

    // load_file('TVideo11.sql');
    // load_data('TVideo11', FALSE); //Usar en updates
    // load_data('TVideoFile11', FALSE);
    // load_data('TVideoCountries11', FALSE);
    // load_data('TVideoCompanies11', FALSE);
    // load_data('TVideoTipo11', FALSE);
    // load_data('TVideoSecciones11', FALSE);
